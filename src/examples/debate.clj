(ns examples.debate
  "An example of listening to two agents debate each other. We will build on the example
...
(defn debate
  ...
  (let [as-user (map #(assoc % :role :user)) ;;; Each agent should send their output as a user instead of an assistant
        partner (fn [{:keys [prompt voice instructions]}]
                  (-> (conv/conversation-partner (slurp (io/resource prompt)) nil as-user)
